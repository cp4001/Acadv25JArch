...
namespace Acadv25JArch
{
    ...
    public class DoorPart : INotifyPropertyChanged
    {
        private static int IdCounter = 1;
        public int Index { get; set; }
        private ObjectId ObjID { get; set; }  // Block Id not Blockreference
        ...
        public static List<DoorPart> GetAllBlockParts()
        {
            Document doc = Application.DocumentManager.MdiActiveDocument;
            Database db = doc.Database;
            Editor ed = doc.Editor;

            List<DoorPart> blockParts = new List<DoorPart>();
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                var ents = JEntity.GetEntityAllByTpye<Entity>(JEntity.MakeSelFilter("INSERT", "Door"));                //(JEntity.MakeSelFilter("INSERT", "ElecPart"));
                if (ents == null) return null;
                foreach (Entity ent in ents)
                {
                    blockrefs.Add(ent as BlockReference);
    ...
    public class RoomPart : INotifyPropertyChanged
    {
        ...
        public RoomPart(Polyline poly)
        {
            ...
            this.Floor = JXdata.GetXdata(poly, "Floor");
        ...
        public string GetWallText(Polyline poly)
        {
            ...
            foreach (var line in lines)
            {
                ...
                foreach (var br in brs)
                {
                    ...
                    //Check Doors
                    if ((JXdata.GetXdata(br, "Door") != null)) // Door이면   외벽문 내벽문  구분 없이 추가 
                    {
