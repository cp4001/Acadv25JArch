...
namespace Acadv25JArch
{
    ...
    public class RoomCalc
    {
        ...
        public void Cmd_Poly_Set_RoomPoly()
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                List<Entity> targets = JEntityFunc.GetEntityByTpye<Entity>("Room 대상  poly를  선택 하세요?", JSelFilter.MakeFilterTypes("POLYLINE,LWPOYLINE"));//, JSelFilter.MakeFilterTypes("LINE,POLYLINE,LWPOYLINE"));   //DBText-> Text  Mtext-> Mtext  
                if (targets == null) return;
                var btr = tr.GetModelSpaceBlockTableRecord(db);
                //사용할 XData 미리 Check
                tr.ChecRegNames(db, "Archi,Room,Disp");
