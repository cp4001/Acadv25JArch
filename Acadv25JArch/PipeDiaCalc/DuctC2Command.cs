        ...
        public void Cmd_DuctC2()
        {
            ...
            try
            {
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    if (!TryReadWidth(cc, out double Wcc)) { ed.WriteMessage("\n[E02] cc XData \"a\"(폭) 를 읽을 수 없습니다."); return; }
                    ...
                    if (Waa <= Wbb)
                    {
                        ed.WriteMessage("\n[E04] 축소 조건(W_aa > W_bb)을 만족하지 않습니다.");
                        return;
                    }
                    ...
                    {
                        ed.WriteMessage("\n[E05] aa/bb/cc 의 한쪽 끝점이 교차점 X 와 일치하지 않습니다.");
                        return;
                    }
                    ...
                    {
                        ed.WriteMessage("\n[E07] bb 가 aa 와 동일선상(반대 방향)이 아닙니다.");
                        return;
                    }
                    ...
                    {
                        ed.WriteMessage("\n[E08] bb 길이가 부족합니다 (B2 를 지나야 합니다).");
                        return;
                    }
                    if (ccFar.DistanceTo(X) < c2.DistanceTo(X) + JunctionTol)
                    {
                        ed.WriteMessage("\n[E08] cc 길이가 부족합니다 (c2 를 지나야 합니다).");
                        return;
                    }
                    if (aaFar.DistanceTo(X) < Hcc + Lcol + JunctionTol)
                    {
                        ed.WriteMessage("\n[E09] aa 길이가 부족합니다 (컬러 사선 시작점보다 길어야 합니다).");
                        return;
                    }
                    ...
                    ed.WriteMessage($"\nDuct_C2 완료: aa 외곽선 {created}개 생성, Green 연장 2선, bb 끝점 B2 이동, cc 3분할 " +
