...
{
    /// <summary>
    /// Duct_C2E — Duct_C2 와 동일한 동작이되, cc 컬러의 상류측 마감이 45° 사선이 아니라
    /// 90° 수직선(직선)인 변형. 컬러 좌측이 S1–S2 수직선이 되어 컬러가 좌우 대칭 사각형이 된다.
    /// 수평 기준선 aa(Green) + 동일선상 bb(Red) + 직각 상향 cc(Cyan) 선택 →
    ...
    public class DuctC2ECommand
    {
        ...
        [CommandMethod("Duct_C2E", CommandFlags.UsePickSet)]
        public void Cmd_DuctC2E()
        {
        ...
        /// bb 끝점 B2 이동 + cc 3분할을 수행한다. 컬러 상류측 마감은 90° 수직선.
        ...
        public bool TryApply(Transaction tr, Database db, Line aa, Line bb, Line cc, out string message)
        {
            ...
            // 교차점 X = cc 위 임의의 점(cc.StartPoint)을 aa(연장 포함)에 수직 투영
            ...
            double Hbb = Wbb / 2.0;
            double Hcc = Wcc / 2.0;
            double hh = (Waa - Wbb) / 2.0;                                      // 편측 낙차
            double Lred = hh / System.Math.Tan(30.0 * System.Math.PI / 180.0); // B1–B2, 사선 수평과 30°
            double Lcol = Wcc / 4.0;                                            // c1–c2 = 컬러 높이

            // 분할·연장 점
            Point3d B1 = X + dirBB * (1.5 * Wcc);   // 리듀서 시작 (X–B1 = 1.5 × W_cc)
            ...
            if (aaFar.DistanceTo(X) < Hcc + JunctionTol)
            {
                message = "[E09] aa 길이가 부족합니다 (컬러 좌측 수직선보다 길어야 합니다).";
                return false;
            }

            // 외곽선 정점 — 본체 2 + 좌측 마감 1 + 컬러 3 + 리듀서 4 (총 10선)
            Vector3d up = dirCC;
            ...
            Point3d S1 = X + dirAA * Hcc + up * Haa;            // 컬러 좌측 수직 하단 (90° — Duct_C2 의 45° 사선 대체)
            Point3d S2 = X + dirAA * Hcc + up * (Haa + Lcol);   // 컬러 상단 좌측
            ...
            // Green(aa) 연장 2선 생성 (원본 유지, aa 속성·XData 승계)
            CreateBranchSegment(tr, btr, db, X, B1, aa);
            CreateBranchSegment(tr, btr, db, B1, B2, aa);

            // Red(bb) X측 끝점을 B2 로 이동 (분할 없음)
            MoveEnd(bb, X, B2);

            // cc 3분할 (신규 X–c1, c1–c2 + 원본 X측 끝점 c2 이동)
            ...
            created += AddOutlineLine(tr, btr, db, T1, U1); // 좌측 끝 수직 마감
            created += AddOutlineLine(tr, btr, db, S1, S2); // 컬러 좌측 수직 (90°)
            created += AddOutlineLine(tr, btr, db, S2, S3); // 컬러 상단 수평
            created += AddOutlineLine(tr, btr, db, S3, S4); // 컬러 우측 수직
            created += AddOutlineLine(tr, btr, db, T2, U2); // 리듀서 B1 수직 (폭 W_aa 이음선)
            ...
            message = $"Duct_C2E 완료: aa 외곽선 {created}개 생성, Green 연장 2선, bb 끝점 B2 이동, cc 3분할 " +
                      $"(Waa={Waa}, Wbb={Wbb}, Wcc={Wcc}, hh={hh}, B1-B2={Lred:F2}, 컬러 좌측 90°).";
            return true;
        }
