...
{
    /// <summary>
    /// Duct_C1E — Duct_C1 과 동일한 동작이되, Tab(분기 리듀서) 상단 마감을 **분기별로** 고르는 변형.
    /// 규칙(2026-09-09 확정): **Leaf 분기의 Tab 은 사선 없이 90° 직선**, Mid 분기는 45° 사선.
    /// 90° 쪽 Tab 은 상단이 분기축과 나란해져 폭 W_bb / W_cc 의 상하 대칭 사각형이 된다.
    /// 대화형 명령은 bb·cc 가 모두 Leaf 인 경우를 대상으로 하므로 양쪽 다 90° 로 처리하고,
    /// 한쪽만 Leaf 인 혼합 형상은 트리 오케스트레이터가 bbSlant/ccSlant 로 지정한다.
    /// 기준선 aa(Green) + 교차점 X 좌우 직각 bb(Red)/cc(Cyan) 선택 →
    /// aa 덕트 외곽선(Yellow, 레이어 "Duct_OutLine") 9개 생성 + bb/cc 를 각각 3개 Line 으로 분리.
    /// 모든 계산은 월드축이 아니라 선택 선의 방향벡터(dirAA/dirBB/dirCC) 기준으로 수행.
    /// </summary>
    public class DuctC1ECommand
    {
        private const double JunctionTol = 1e-3;  // X 접합점 일치 거리
        private const double PerpTol = 0.02;      // aa⊥bb, aa⊥cc 90° 판정 (약 ±1.15°)
        private const double BottomDrop = 200.0;  // 하부 마감 추가 깊이(고정값)
        private const int Yellow = 2;             // 외곽선 ACI
        private const string OutlineLayer = "Duct_OutLine";

        [CommandMethod("Duct_C1E", CommandFlags.UsePickSet)]
        public void Cmd_DuctC1E()
        {
            ...
            try
            {
                ...
                {
                    // aa → bb(좌) → cc(우) 순서로 1개씩 선택
                    if (!PickLine(ed, tr, "기준선(aa)을 선택하세요?", out Line aa)) return;
                    if (!PickLine(ed, tr, "좌측 기준선(bb, Leaf)을 선택하세요?", out Line bb)) return;
                    if (!PickLine(ed, tr, "우측 기준선(cc, Leaf)을 선택하세요?", out Line cc)) return;

                    // 대화형 진입은 bb·cc 가 모두 Leaf 인 경우 — 양쪽 Tab 모두 90° 직선
                    if (!TryApply(tr, db, aa, bb, cc, false, false, out string message))
                    {
                        ed.WriteMessage("\n" + message);
                        return;
                    }
        ...
        /// <summary>
        /// aa(기준선) + bb(좌)/cc(우) 로부터 aa 외곽선 9개 생성 + bb/cc 3분할을 수행한다.
        /// bbSlant/ccSlant 가 false 인 분기는 Tab 상단을 분기축과 나란한 90° 직선으로 마감하고(Leaf),
        /// true 인 분기는 Duct_C1 과 동일한 45° 사선으로 마감한다(Mid).
        /// 대화형 선택 없이 Line 객체를 직접 받아 검증~계산~생성까지 처리 — 트리 오케스트레이션에서 재사용.
        /// 검증 실패 시 false 반환(엔티티 생성 없음, Transaction 은 변경 없음), 성공 시 true + 결과 메시지.
        /// </summary>
        public bool TryApply(Transaction tr, Database db, Line aa, Line bb, Line cc,
                             bool bbSlant, bool ccSlant, out string message)
        {
            message = "";
            ...
            // 교차점 X = aa 위 임의의 점(aa.StartPoint)을 cc(연장 포함)에 투영
            ...
            // aa/bb/cc 의 X 측 끝점이 실제로 X 와 일치하는지 확인
            ...
            double Hbb = Wbb / 2.0;
            double Hcc = Wcc / 2.0;
            double dBb = Wbb / 4.0; // Δbb — Tab 돌출 길이
            double dCc = Wcc / 4.0; // Δcc — Tab 돌출 길이
            ...
            // bb/cc 길이 충분성 검증 (분할점을 지나 원래 끝점까지 여유가 있어야 함)
            ...
            // Tab 상단이 aa 외곽 벽에서 시작하는 높이 — 사선(Mid)이면 H+Δ, 90° 직선(Leaf)이면 H
            double bbTop = Hbb + (bbSlant ? dBb : 0.0);
            double ccTop = Hcc + (ccSlant ? dCc : 0.0);

            // aa 길이(y_top) 검증 — Tab 상단 시작 높이보다 길어야 함
            double yTop = aaFar.DistanceTo(X);
            if (yTop < System.Math.Max(bbTop, ccTop) + JunctionTol)
            {
            ...
            // 외곽선 정점 12점 — 연속 수직 2 + 하부 수평 + Tab(상단·끝단 수직·하단 직선) x 2
            Vector3d up = dirAA;
            Point3d PA = b1 + up * yTop;          // 좌측 수직 상단      (−Haa, yTop)
            Point3d PB = b1 + up * bbTop;         // bb Tab 상단 시작    (−Haa, +Hbb[+Δbb])
            Point3d PC = b2 + up * Hbb;           // bb Tab 상단 끝      (−Haa−Δbb, +Hbb)
            Point3d PD = b2 - up * Hbb;           // bb Tab 하단 끝      (−Haa−Δbb, −Hbb)
            Point3d PE = b1 - up * Hbb;           // bb Tab 하단 시작    (−Haa, −Hbb)
            Point3d PF = b1 - up * yBot;          // 좌측 수직 하단      (−Haa, −yBot)
            Point3d PG = c1 - up * yBot;          // 우측 수직 하단      (+Haa, −yBot)
            Point3d PH = c1 - up * Hcc;           // cc Tab 하단 시작    (+Haa, −Hcc)
            Point3d PI = c2 - up * Hcc;           // cc Tab 하단 끝      (+Haa+Δcc, −Hcc)
            Point3d PJ = c2 + up * Hcc;           // cc Tab 상단 끝      (+Haa+Δcc, +Hcc)
            Point3d PK = c1 + up * ccTop;         // cc Tab 상단 시작    (+Haa, +Hcc[+Δcc])
            ...
            // bb/cc 분리 (신규 Line 2개 + 원본 X측 끝점 이동), XData/속성 승계
            SplitBranch(tr, btr, db, bb, X, b1, b2);
            SplitBranch(tr, btr, db, cc, X, c1, c2);

            // aa 외곽선(Yellow) 9개 — 연속 수직 2 + 하부 수평 1 + Tab 3선 x 2
            int created = 0;
            ...
            created += AddOutlineLine(tr, btr, db, PB, PC); // bb Tab 상단 (Leaf=90° 직선 / Mid=45° 사선)
            created += AddOutlineLine(tr, btr, db, PC, PD); // bb Tab 끝단 수직
            created += AddOutlineLine(tr, btr, db, PD, PE); // bb Tab 하단 직선
            created += AddOutlineLine(tr, btr, db, PK, PJ); // cc Tab 상단 (Leaf=90° 직선 / Mid=45° 사선)
            created += AddOutlineLine(tr, btr, db, PJ, PI); // cc Tab 끝단 수직
            created += AddOutlineLine(tr, btr, db, PI, PH); // cc Tab 하단 직선

            message = $"Duct_C1E 완료: aa 외곽선 {created}개 생성, bb/cc 각 3분할 " +
                      $"(Waa={Waa}, Wbb={Wbb}, Wcc={Wcc}, bb Tab={(bbSlant ? "45° 사선" : "90° 직선")}, cc Tab={(ccSlant ? "45° 사선" : "90° 직선")}).";
            return true;
        }
